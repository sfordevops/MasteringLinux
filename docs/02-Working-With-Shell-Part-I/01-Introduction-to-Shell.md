...
- A user **`shiva`** home directory created under **`/home/shiva`**, where **`/home`** is a system created home directory that contains the home directories for almost all users in the linux system. 
- The name of the home directory is by default identical to the name of the user, hence **`shiva's`** home directory is **`/home/shiva`**.
- Remember the home directory is unique for each user. Another user called **`prasad`** will have a different home directory which by default created under **`/home/prasad`**.
...
e- You can configure the command prompt to show whatever you want, such as the **`hostname`** , **`date`** or **`time`**. 
